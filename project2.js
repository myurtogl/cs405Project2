...
class MeshDrawer {
	...
	setTexture(img) {
		...
		if (isPowerOf2(img.width) && isPowerOf2(img.height)) {
			...
			// Set texture wrapping to CLAMP_TO_EDGE for NPOT textures.
			gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
			gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
	
			// Set texture filtering (here using LINEAR for simplicity, but you can choose as per your requirement).
...
const meshFS = `
    ...
    void main() {
        ...
        vec4 finalColor = showTex ? (enableLighting ? (diffColor + ambColor) : texColor) : vec4(1.0, 0, 0, 1.0);
